    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <form action="{{ route('corridors.update', $corridor) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea
                            ...
                            name="description"
                            rows="3"
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-cj-morado-medio focus:ring focus:ring-cj-morado-claro @error('description') border-red-500 @enderror">{{ old('description', $corridor->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="url"
                               id="logo_url"
                               name="logo_url"
                               value="{{ old('logo_url', $corridor->logo_url) }}"
                    ...
                    <div class="mb-6">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   name="is_active"
                                   {{ old('is_active', $corridor->is_active) ? 'checked' : '' }}
