        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="bg-white/90 backdrop-blur-lg rounded-2xl shadow border border-white/50 p-5">
                    <p class="text-xs font-semibold text-cj-texto-claro uppercase tracking-wider mb-1">Transferencias</p>
                ...
                <div class="bg-gradient-to-br from-cj-morado-profundo to-cj-morado-medio text-white rounded-2xl shadow p-5">
                    <p class="text-xs font-semibold opacity-80 uppercase tracking-wider mb-1">Total recibido (Perú)</p>
                ...
                <div class="bg-gradient-to-br from-cj-turquesa to-teal-400 text-white rounded-2xl shadow p-5">
                    <p class="text-xs font-semibold opacity-80 uppercase tracking-wider mb-1">Total enviado (VE)</p>
            ...
            <div class="bg-white/90 backdrop-blur-lg rounded-2xl shadow-xl border border-white/50 overflow-hidden">
                <div class="p-5 border-b border-gray-100 flex items-center justify-between">
                    <div>
                        <h3 class="font-bold text-cj-morado-profundo">Transferencias bancarias</h3>
                        <p class="text-xs text-cj-texto-claro mt-0.5">{{ $start }} al {{ $end }} — Solo transferencias en estado "en proceso" o "completadas"</p>
                    </div>
                </div>

                @if($transactions->isEmpty())
                <div class="p-12 text-center">
                    <p class="text-cj-texto-claro text-lg">No hay transferencias bancarias en este período.</p>
                </div>
                @else
                    ...
                    <table class="min-w-full text-sm">
                        <thead class="bg-gray-50 border-b border-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-bold text-cj-texto-claro uppercase tracking-wider">Titular (Perú)</th>
                                <th class="px-4 py-3 text-left text-xs font-bold text-cj-texto-claro uppercase tracking-wider">Banco origen</th>
                                <th class="px-4 py-3 text-left text-xs font-bold text-cj-texto-claro uppercase tracking-wider">Nº cuenta origen</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors {{ $tx->operation_number ? '' : 'bg-yellow-50/40' }}">
                                ...
                                <td class="px-4 py-3 text-cj-texto">
                                    ...
                                    @if($tx->sender_dni)
                                    <div class="text-xs text-cj-texto-claro font-mono">DNI: {{ $tx->sender_dni }}</div>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-cj-texto">{{ $tx->sender_bank ?? '—' }}</td>
                                <td class="px-4 py-3 font-mono text-xs text-cj-texto">{{ $tx->sender_account_number ?? '—' }}</td>
                                <td class="px-4 py-3">
                        ...
                        <tfoot class="bg-gray-50 border-t-2 border-gray-200">
                            <tr>
                                <td colspan="6" class="px-4 py-3 font-bold text-cj-texto text-sm">TOTAL</td>
